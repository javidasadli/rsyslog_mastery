...
version=2
#rule=:%<field_name>:<field_type>{<parameters>}%
rule=:<%rsyslog_priority:number%>1 %rsyslog_time:date-rfc5424% %rsyslog_host:word% %rsyslog_app:word% %rsyslog_procid% %rsyslog_msgid:word% [version="5" environment=%rsyslog_env:op-quoted-string% class=%rsyslog_class:op-quoted-string%] %rsyslog_message:rest%